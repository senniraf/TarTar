...
typedef enum {AUT_FMT, BCG_FMT, DIR_FMT, GCD_FMT, GCF_FMT, FSM_FMT, XML_FMT } lts_format_t;
...
static lts_format_t get_fmt(const char*name){
    ...
    if (!strcmp(extension,"xml")) return XML_FMT;
...
lts_file_t lts_file_create(const char* name,lts_type_t ltstype,int segments,lts_file_t settings){
    ...
    switch(fmt){
        ...
        case XML_FMT: name = xml_file_open(name);
        	return gcf_file_create(name,ltstype,segments,settings);
...
lts_file_t lts_file_open(const char* name){
    ...
    switch(fmt){
        ...
        case XML_FMT: return gcf_file_open(name);
