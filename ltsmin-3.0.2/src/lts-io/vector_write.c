...
static void write_init(lts_file_t file,int seg,void* state){
    ...
    //xml_write_state(file, seg, state, 0);
...
static void write_edge(lts_file_t file,int src_seg,void* src_state,
  ...
  //todo:  if (file->edge_lbl) DSwriteStruct(file->edge_lbl[i],labels);

    //xml_write_edge(file,src_seg, src_state, dst_seg,dst_state, labels);
}
...
lts_file_t xml_file_create(const char* name, lts_type_t ltstype, int segments,
		lts_file_t settings) {
    hre_context_t global = lts_file_context(settings) ? lts_file_context(settings) : HREglobal();
    int me=HREme(global);
    int peers=HREpeers(global);
