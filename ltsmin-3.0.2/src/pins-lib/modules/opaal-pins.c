...
const int   (*get_label_count)  ();
const char* (*get_label_name)  (int g);

//const char* (*getStateName)(void *in, void *out);
//const char* (*getLatticeName)(void *in);
...
sl_long_p (model_t model, int label, int *state)
{
	...
	printf("acc: %d", ctx->accepting_state_label_idx);
	printf("lattice: %d", ctx->lattice_label_idx);
    if (label == ctx->accepting_state_label_idx) {
    	...
    	//return (int)getStateName(state, 0);
...
opaalLoadDynamicLib(model_t model, const char *filename)
{
    ...
    get_label_name = (const char*(*)(int))
    RTdlsym( filename, dlHandle, "opaal_get_label_name" );
    get_label_count = (int(*)())
    RTdlsym( filename, dlHandle, "opaal_get_label_count" );

   // getStateName = (const int*(*)(int))
   // RTdlsym( filename, dlHandle, "getStateName");
   // getLatticeName = (const int*(*)(int))
   // RTdlsym( filename, dlHandle, "getLatticeName");
...
opaalLoadGreyboxModel(model_t model, const char *filename)
{
    ...
    //load all edge label names
    int count =	get_label_count();
    lts_type_set_edge_label_count(ltstype, count);
    for(int i = 0; i< count; i++)
    {
    	const char* label = get_label_name(i);
    	lts_type_set_edge_label_name(ltstype,i, label);
    }
