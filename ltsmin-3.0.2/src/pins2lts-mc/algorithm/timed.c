...
ta_handle (void *arg, state_info_t *successor, transition_info_t *ti, int seen)
{
    ...
    if(ctx->initial != successor)
    	Buchi_addTransition(ctx->model, ctx->state, ti, successor);
    ctx->counters->trans++;
    (void) seen;
}
            ...
            while ((state_data = isba_pop_int (sm->queues[current])) &&
                ...
                if (grab_waiting(ctx, state_data)) {
                    ...
                    if (EXPECT_FALSE(loc->lts && write_state)){
                        ...
                        Buchi_addState(ctx->model, pins_state);

                        //char* name = getStateName(ctx->model, ctx->state);
                        //printf("%s\n", name);
                    }
                }
            }
    ...
    if (0 == ctx->id) { // only w1 receives load, as it is propagated later
        if ( Strat_PBFS & strategy[0] ) {
            if (ctx->local->lts != NULL) {
                ...
                xml_set_init(ctx->model, initial);
                Buchi_addState(ctx->model, initial);
            }
        }
