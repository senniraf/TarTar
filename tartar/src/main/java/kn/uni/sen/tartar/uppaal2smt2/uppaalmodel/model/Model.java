...
public class Model
{
	...
	private String getInstanceTemplate(String templName)
	{
		if (system == null)
			return "";
		String sys = Helper.deleteComment(system).replace(" ", "");
	...
	public List<String> getClockList()
	{
		...
		String decl = Helper.deleteComment(globalDeclaration);
	...
	public List<VariableModel> parseVariable(String val)
	{
		...
		String[] list = Helper.splitPattern(val + ";", "; =,\n");
		for (String s : list)
		{
			if (bVal)
			{
	...
	{
		decl = Helper.deleteComment(decl);
	...
	private int getMaxContraint(String text)
	{
		...
		String[] list = Helper.splitPattern(text, " <=>&|!");
		int max = 0;
		for (String str : list)
		{
			if (str.isBlank())
